...
--
-- lazy requires this to be defined before loading lazy
-- Is this workaround still necessary in nvim?
-- nnoremap <Space> <nop>
-- map <Space> \
-- let g:mapleader="\\"
...
if vim.g.vscode == nil then
  ...
  if has_compiler() then
    require'nvim-treesitter.configs'.setup {
      ...
      ensure_installed = {"lua", "python"}, -- Or "all" to install parsers for all supported languages
...
-- SYSTEM CLIPBOARD always, both local and ssh:
--
-- New approach: Don't use unnamedplus. It's discouraged by justinmk and I think I could do without, even though I have a preference for this.
-- Use new mappings to directly interact with the "+ clipboard.
-- Without unnamedplus OSC52 functions just work.
--
-- Outdated after realizing this breaking regular yank/delete etc.
--
-- This has been a long journey... Tested with kitty, alacritty
...
if is_executable("kitten") then
  vim.g.clipboard = {
    ...
    paste = {
      ...
      -- Bad, because regular yanks inside of vim don't write to xclip
...
-- emx style
-- I want words to be delimited by _ except for pressing *
-- vim.opt.iskeyword = "@,48-57,_,192-255" -- default
vim.opt.iskeyword:remove("_")

local function star_with_underscore()
    vim.opt.iskeyword:append("_")
    vim.cmd("normal! *")
    vim.opt.iskeyword:remove("_")
end
vim.keymap.set("n", "*", star_with_underscore, { noremap = true, silent = true })
